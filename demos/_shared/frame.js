...
window.DemoFrame = (function(){
  ...
  var ide;
  /* frame.js 自己的位置：用来就近引入 kit 的 ai.css（智能色、状态机、组件），demo 不必各自再引 */
  var BASE = (function(){ var c = document.currentScript; return c ? c.src.replace(/[^\/]*$/, '') : ''; })();
  function injectAiCss(){
    if(!BASE || document.querySelector('link[href*="ai.css"]')) return;
    var l = document.createElement('link'); l.rel = 'stylesheet'; l.href = BASE + '../../deveco-intui-kit/src/ai.css';
  ...
  function headerHTML(o){
    return '<div class="hdr">'
      ...
      + '</div><div class="zone center" id="hdr-center">'+(o.center||'')+'<span id="hdr-agent"></span></div><div class="zone right">'
      + (o.workspace ? '<a class="hbtn" id="hdr-ws" href="'+esc(o.workspace)+'" title="在 Agent 工作台里看这个任务：任务、在等你、结果、改动、证据"><svg class="i"><use href="#i-layout"/></svg><span>工作台</span></a><span class="sep"></span>' : '')
  ...
  /* ---------- init ---------- */
  /* ---------- Agent 胶囊：顶栏中间，状态取 ai.css 状态机 ----------
     DemoFrame.agent('generating', 'Agent·列表 在改 ListPage.ets', {queue:2, onClick:fn})
     state：idle 待命 / thinking / tool / generating / waiting 在等你（琥珀）/ verifying / done / error；
     queue：统一「在等你」队列里还有几件（多 Agent 时胶囊只推最挡路的一件，其余显示「还有 N 件」）；传 null 清空胶囊。 */
  var agentFn = null;
  function agent(state, html, o){
    var c = document.getElementById('hdr-agent'); if(!c) return;
    o = o || {};
    if(state == null){ c.innerHTML = ''; agentFn = null; return; }
    agentFn = o.onClick || null;
    var q = o.queue ? '<span class="t">· 还有 ' + o.queue + ' 件</span>' : '';
    c.innerHTML = '<span class="ai-status" data-state="' + esc(state) + '" role="' + (agentFn ? 'button' : 'status') + '" tabindex="' + (agentFn ? '0' : '-1') + '">'
      + '<svg class="i ai-ico" aria-hidden="true"><use href="#i-sparkle"/></svg><span class="lbl">' + (html || '') + '</span>' + q + '</span>';
    var el = c.firstChild;
    if(agentFn){ el.style.cursor = 'pointer'; el.addEventListener('click', agentFn); el.addEventListener('keydown', function(e){ if(e.key === 'Enter' || e.key === ' '){ e.preventDefault(); agentFn(); } }); }
  }
  ...
  function init(opts){
    ...
    var o={project:project,avatar:opts.avatar||project.replace(/[^A-Z]/g,'').slice(0,2)||project.slice(0,2),vcs:opts.vcs||'Version Control',
      ...
      workspace:opts.workspace===false?'':(opts.workspace||BASE+'../31-agent-workspace/index.html')};
  ...
  return {agent:agent,init:init,tree:tree,code:code,tabs:tabs,crumbs:crumbs,setSide:setSide,esc:esc,FILES:FILES,$:$,$$:$$};
})();
